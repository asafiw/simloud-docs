...
export default function rehypeStarryNightWithCopyAndCollapse() {
...
function addStarryNight(starryNight, node, index, parent) {
  ...
  if (expandable) {
    attachHighlightWithToggler(starryNight, stringContent, parent, scope, index)
  } else {
    attachHighlight(starryNight, stringContent, parent, scope, index)
  }
}

function attachHighlightWithToggler(starryNight, stringContent, parent, scope, index) {
  const preChildrenCompacted = starryNight.highlight(
    getCompactedString(stringContent),
    scope
  ).children
  const preChildrenExpanded = starryNight.highlight(stringContent, scope).children
